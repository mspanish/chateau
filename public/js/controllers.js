    ...
    $http.get('/api/table', {params: {db: $scope.db, table: $scope.table, skip: $scope.skip, limit: $scope.limit, order: $routeParams.order, ascDescValue: $scope.ascDescValue}}).
        success(function(data) {
            if (data.error != null) {
                ...
                /* Stacey Tipton Reiman add color squares for any colors inside of an array! */
                        setTimeout(function(){
                       $('.array').each(function(){
                          var cols = $(this).text();
                        ...
                        /* FILTER FOR viewing ONLY art with COLORS!! */
                          if (cols[0] == "empty"){
                           // console.log('GOTCHA!!')
                           ...
                           $.each(cols, function(key,value){
                            ...
                            var primary = element.parent().siblings().first().next().text()
                            //console.log('primary = '+primary)
                            var numberRange = _.range(10000);
                              var ranNum = _.sample(numberRange);
                             var col = value//.replace('[','').replace(']','');
                             // console.log('col is '+col)
                             element.css({'display':'table','color':'#ffffff','overflow-x': 'scroll','height': '23px'})

                              element.prepend('<div id="'+ranNum+'" data-color="'+col+'" class="colory"></div>')
                              var obj = {'border': 'solid 1px #666666','width':'25px','height':'18px', 'margin-right': '3px', 'backgroundColor':col, 'float':'left'}
                             // console.log(JSON.stringify(obj))
                            // console.log('primaryKey: '+$scope.primaryKey)
                             
                              $ ('#'+ranNum).css(obj).click(function(){
                                var newCols = _.without(cols, value)
                                console.log(JSON.stringify(newCols))
                                changeColors(primary,newCols)
                              })
                            } // end if we have a color
                            if (key == cols.length-1) {
                                if (gotColor) {

                                  element.prepend('<input id="col_'+ranNum+'" value="#ffffff" class="col jscolor"/><button id="btn_'+ranNum+'" class="colorAdd">add</button>')  
                                  $ ('#btn_'+ranNum).click(function(){
                                    var colorAdded = $('#col_'+ranNum).val().toLowerCase();
                                    console.log('newCol is '+colorAdded)
                                    var newerCols = cols.concat([colorAdded])
                                   // console.log(JSON.stringify(newerCols))
                                // we can actually use the same function
                                    changeColors(primary,newerCols)
                                  })
                                }
                            }
                            }) // end each cols
                        }) // end each array
                       //$('.array').css('background-color','#ff0000')
                       // $('body').css('background-color','#ffff00')
//if (!jscolorActive) {
 //activateJSColor();  
  $('.jscolor').colorPicker().each(function(idx, elm) {
    if (!elm.value) {
        $(elm).data('colorMode', 'HEX');
    }
});
// jscolorActive = true;
 //console.log('activated js color')
//}
// end table loading
                        },300)
    ...
    function changeColors(primary,newCols) {
        var data = {
            db: $scope.db,
            table: $scope.table,
            primary: primary,
        ...
        $http.post('/api/change_colors', data).
            success(function(data) {
                if (data.error != null) {
                    h.handleError(data.error);
                }
...
//            console.log('flattened doc is '+JSON.stringify(flattened_doc))
